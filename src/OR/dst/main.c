...
// csvへの出力_関数(受信・送信パケット(1・2番目))(srcHop , destHop)
...
// csvへの出力_関数(u , parameter)
void output_data_csv_u(FILE *fp,or_data_packet_t* hdr,char* filename){
    ...
    fprintf(fp,"%lf,%lf",hdr->u,hdr->parameter);
    fclose(fp);
}
    ...
    if(current && is_same_seq(current->seq,seq)){
        ...
        if(current->flag == DATA){
            ...
            if(r_current->hop >= data_p->destHop){
                ...
                output_data_csv_u(fp_rx,data_p,rx_filename);
            // パターン3：ACK送信しない
            }else{
                //3
...
void handleFirstReceive(routing_table_entry_t* current, packet_table_entry_t* p_current, mac_frame_header_t* packet_p){
    ...
    if(0 < diff){
        printf("handleFirstReceive関数if分岐(0<diff)\n"); 
        ...
        if(exTravDist == data_p->srcHop){
            printf("handleFirstReceive関数if分岐(exTravDist == data_p->srcHop)\n"); 
...
void handleDuplicationReceive(routing_table_entry_t* current, packet_table_entry_t* p_current, mac_frame_header_t* packet_p){
    ...
    // ホップ数が一致しているか判定
    if(exTravDist == data_p->srcHop){
        printf("handleDuplicationReceive関数if分岐(exTravDist == data_p->srcHop)\n"); 
        // 重複して受信した場合
        if(p_current->receiveCount == 1){
            ...
            if(current->lastSeqNum == packet_p->seqNum){
                printf("handleDuplicationReceive関数if分岐(current->lastSeqNum == packet_p->seqNum)\n"); 
                double randDiff = data_p->u - current->lastPassiveRandU;
                printf("randDiff = %lf\n", randDiff);
    ...
    // 受信パケットがDATAの場合
        ...
        // 宛先ノードが自分ではない場合
            ...
            if(current){
                ...
                if(p_entry->flag != ACK){
                    // 転送待機時間の算出
                    double bo = OR_calculate_backoff(data_p->destHop,current->hop,data_p);
                    get_time(&after_backoff);
                    print_time(&after_backoff);
                    insert_backoff(packet_p->SourceAddr,packet_p->seqNum,bo,after_backoff);
                // 1回目に再受信した場合
                }else if(p_entry->transmitted == TRUE && p_entry->receiveCount == 0){
                    handleFirstReceive(current, p_entry ,packet_p);
                // 2回目に再受信した場合
                }else if (p_entry->transmitted == TRUE && p_entry->receiveCount == 1){
...
void tx_file_open(char* file_name){
    ...
    fprintf(fp_tx,"tx_time,time,nsec,,Type,srcAddr,senderAddr,destAddr,length,seq,tx_checksum\n");
    fclose(fp_tx);
}
